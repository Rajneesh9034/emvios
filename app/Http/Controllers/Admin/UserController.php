...
class UserController extends Controller
{
  ...
  public function available_balance()
   ...
   public function auto_withdrwal(Request $request)
        {

          $id= $request->id; // or any params
          $update_data['auto_withdrawal']= ($request->status=="ON")?1:0;
          $user =  user::where('id',$id)->update($update_data);

        $notify[] = ['success', 'Auto Withdrawal '.$request->status.' successfully'];
        return redirect()->back()->withNotify($notify);
      }
  ...
  public function make_active(Request $request)
    {
      ...
      if($request->status=="Inactive")
      {
             ...
             sendEmail($userData->email, 'Account Deactivated Due to Settlement Issue', [
                'name' => $userData->name,
                'viewpage' => 'inactiveMail',
  
            ]);
      }

    $notify[] = ['success', ' Status  '.$request->status.' updated successfully'];
    return redirect()->back()->withNotify($notify);
  }
