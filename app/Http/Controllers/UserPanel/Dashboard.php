...
class Dashboard extends Controller
{
 ...
 $incomeData = Income::selectRaw('DATE(created_at) as date, SUM(comm) as total')
        ->where('user_id', $user->id)
        ->groupBy('date')
        ->orderBy('date', 'asc')
        ->take(7)
        ->get();
          $dates = $incomeData->pluck('date');
    $totals = $incomeData->pluck('total');
  ...
  $this->data['chartDates'] = $dates;
    $this->data['chartTotals'] = $totals;
 ...
 public function checkPaymentfund(Request $request)
    {
     date_default_timezone_set("Asia/Kolkata");   //India time (GMT+5:30)
