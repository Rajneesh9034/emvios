 ...
 foreach ($allResult as $key => $value)
 {
  ...
  $allResult=User::where('active_status','Active')->get();
    ...
    if ($allResult)
    {
        ...
        $userName = $value->username;
        $userRank = $value->rank;
        $pckage = $value->package;
        
        $total = Investment::where('user_id',$userID)->where('status','Active')->sum("amount");
        
        if($total!=$pckage)
        {
               echo "ID:".$userName." Package:".$pckage." Total:".$total."<br>";
            User::where('id',$value->id)->update(['package'=>$total]); 
        }
...
public function dynamicupicallback(Request $request)
{
    try {
        ...
        $validAddresses = [
            "0x622b8aA4dcaF747C1CeB9a837f48b24e26F5C7aF",
