        ...
        while ($user_mid!="" && $user_mid!="1"){
              ...
              $pp=0;
               if($sp_status=="Active")
               {
                 if($cnt==1 && $Sposnor_cnt>=$cnt)
                  ...
                  if($cnt>=3 && $cnt<=10 && $Sposnor_cnt>=$cnt)
                  {
                        
                    $pp= $amount*0.30;

                  }  
                   if($cnt>=11 && $cnt<=20 && $Sposnor_cnt>=$cnt)
                  {
                    $pp= $amount*0.20;

                  }  
                  if($cnt>=21 && $cnt<=30 && $Sposnor_cnt>=$cnt)
                  {
                    $pp= $amount*0.10;
           ...
           // dd($total_received);
            ...
            //  }  
              ...
              $idate = date("Y-m-d");
             $total_profit = \DB::table('incomes')->where('user_id',$Sposnor_status->id)->where('remarks','Farming Profit')->where('rname',$rname)->count();
              $user_id_fk=$sponsor;
              if($spid>0 && $cnt<=30){
                if($pp>0 && $total_profit<60){
